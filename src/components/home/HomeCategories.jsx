...
function MenuCategories() {
  ...
  return (
    <div className="max-w-[1050px] mx-auto py-2 sm:py-4 lg:py-6 relative">
      ...
      {/* Mobile: swipeable */}
      <div className="lg:hidden relative px-4 md:px-6">
        <div className="flex overflow-x-auto gap-3 scrollbar-hide py-2 snap-x snap-mandatory">
          {categories.map((cat) => (
            <div
              key={cat.name}
              className="flex flex-col items-center cursor-pointer hover:opacity-80 transition-opacity flex-shrink-0 w-[calc((100vw-2rem-24px)/3)] md:w-[calc((100vw-3rem-24px)/2)] snap-start"
            ...
            >
              ...
              >
                <img src={cat.imgUrl} alt={cat.name} style={{ width: 56, height: 56 }} />
              </div>
              <p className="mt-2 text-center font-bold text-coffee-900 text-[12px] md:text-[14px]">
                {cat.name}
              </p>
            </div>
          ))}
        ...
        >
          {categories.map((cat) => (
            <div
              ...
              onClick={() => handleCategoryClick(cat.name)}
            >
              <div className="rounded-full flex items-center justify-center bg-white shadow-md" style={{ width: 160, height: 160 }}>
                <img src={cat.imgUrl} alt={cat.name} style={{ width: 96, height: 96 }} />
              </div>
